---
title: git learn
date: 2017-06-30 01:50:43
tags:
categories: git
---
# <center>git的学习 </center>
## git是什么？
git是一个版本管理软件，是指同一个文件，想要编写不同版本，和代码管理的一个工具。
###### 使用情况一
在写代码时，突然有一个很好的想法，但是你不确定，这个版本是不是可行，这时你可能会想着去保存原来的一个版本，并且从新去新建一个文件或者文件夹。当你的版本有很多时你都会忘掉你是最初的版本是哪个文件了，而且还需要找来找去的。
这时当你用了git后你会发现这一切就只用一个命令就可以去实现。
###### 使用情况二
在你写代码的时候，上级又给你一个新的小任务，但是你又不得不去停下自己现在的主任务去完成你的小任务，这时又选择重新保存一下吗？你可以选择用git，一个命令去解决这些问题
###### 使用情况三
当一个项目做大了，身为项目负责人的你，怎么将每个人的代码合并起来，并且比对不同点？是用自己的眼睛去观察，和根据自己的经验吗？这些都可以用git去管理
###### 使用情况四
当你写好代码时，出去有点事时，回来你发现你的代码和以前的有些不一样，这时的你应该是怎么办？git还是可以用一条命令去还原你的代码。
***
## git的安装
如果没有什么特别是习惯可以无脑下一步，也可以根据自己的习惯修改安装路径
ps:建议不要取消安装git-GUI,和git-bash方便下面的操作
***
## git的使用
ps:命名一定要用英文，使用中文可能会出现编码乱码的问题。
### 1.初始化仓库
git中有自己的名词，将普通的文件夹和git的文件夹做出区分，把git的文件夹叫做仓库。初始化后，就可以和普通的文件夹一样存放文件。只是仓库可以储存不同的版本，但是普通的文件夹不行。
操作：
在你想要转化成git仓库文件夹中，点击右键选择git-bash输入命令
~~~git
命令为:
	git init
~~~
### 2.添加
当你写好了你的代码的时候想要保存一下，作为一个阶段的完整代码，这时就可以向git发出一个保存的命令，就可以添加到git仓库中。
操作：
在你想要提交的文件夹中，点击右键选择git-bash输入命令
~~~git
命令为：
	git add 【文件名】(ps:一定要加上后缀名)
例如：
	git add hello_word.text 
当要添加多个文件时:
...
~~~
### 3.提交
当你添加完后，这还不是完整的保存了你的版本，这时你还需要，去提交一下你的请求，这样保证了数据的安全性，也给了你一个重新修改的机会
操作:
在你添加到仓库后，继续输入提交命令
~~~git
命令为：
